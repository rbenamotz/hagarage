...
void handleData() {
  ...
  // buildJsonInt(output, "heap", ESP.getFreeHeap());
  // buildJsonInt(output, "flashSize", ESP.getFlashChipSize());
  strcat(output,"}");
...
void handleToggle() {
  ...
  int d = s.toInt();
  write_to_log("Toggle for door %d called from REST API",d);
  toggle_door_state(d - 1);
...
void init_web_server() {
  ...
  server.on("/e", handleEnviroment);
  server.on("/l", handleLog);
